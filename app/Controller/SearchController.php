...
class SearchController extends AppController {
  ...
  function index() {
    if (!empty($this->request->data)) {
      switch ($this->request->data['model']) {
        case 'Note':
          $conditions = array(
            'conditions' => array(
              'Note.note LIKE' => '%'. $this->request->data['keyword'] .'%'  
            ),
        ...
        case 'Contact':
          $conditions = array(
            'conditions' => array(
              'OR' => array(
                'Contact.name LIKE' => '%'. $this->request->data['keyword'] .'%',
                'Contact.value LIKE' => '%'. $this->request->data['keyword'] .'%'  
              )
            ),
        ...
        case 'Lead':
          $conditions = array(
            'conditions' => array(
              'OR' => array(
                'Lead.company LIKE' => '%'. $this->request->data['keyword'] .'%',
                'Lead.name LIKE' => '%'. $this->request->data['keyword'] .'%',
                'Lead.email LIKE' => '%'. $this->request->data['keyword'] .'%',
                'Lead.phone LIKE' => '%'. $this->request->data['keyword'] .'%'
              )
            ),
            'group' => array('Lead.id'),
            'fields' => $this->fields,
